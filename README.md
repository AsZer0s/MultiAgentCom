...
- `GET /status/panel`：打开 WebUI 运维 dashboard，集中查看 readiness、任务拓扑、状态矩阵、告警、HITL 冲突队列、审计、通信日志、Token 成本、沙盒和快照
...
- 存储支持默认内存模式，也可通过 `MULTI_AGENT_STORE_PROVIDER=file` 与 `MULTI_AGENT_DATA_ROOT` 启用文件持久化，或通过 `MULTI_AGENT_STORE_PROVIDER=postgres` 与 `MULTI_AGENT_POSTGRES_DSN` 启用 Postgres 持久化；项目、任务、运行、审计/告警/通信流、artifact 元数据与快照状态可在服务重启后恢复。file-store 使用 typed service-state 适配层读写 `service-state.json`，保存时采用临时文件 + fsync + rename，并在 `/ready` 校验状态 JSON 与 schema version；Postgres provider 使用内嵌 versioned migrations 管理 `schema_migrations`、legacy `service_state` 和 domain projection tables，服务保存时双写关系化投影与 legacy JSONB aggregate，启动时优先从投影表恢复并可从 legacy aggregate backfill；Postgres 模式下 projects/requirements、plans/contracts/tasks 以及 runs/artifacts 的核心读写路径已直接走 SQL projection repository，并在成功后同步内存状态供未迁移流程继续使用。损坏或不支持的 legacy 状态、migration checksum mismatch 或投影表不可读会返回 `not_ready` 而不会被自动覆盖；snapshots/workspaces/context/audit/communication 等完整 repository 化、多实例并发治理与 migrations 外部化仍是后续工作。
...
- WebUI 运维 dashboard 当前支持**集中可视化监控**：可在 `/status/panel` 查看 readiness、项目级任务拓扑 SVG、任务矩阵、Agent 状态汇总、KPI、失败告警、HITL 冲突队列、审计轨迹、通信日志、Token 成本趋势、私有沙盒和时间线快照；拓扑按依赖深度和 assignee agent 分 lane 展示，点击任务节点会按 `taskId` 过滤通信日志与成本条目，HITL 冲突队列可直接 resolve open conflict，状态更新采用 SSE 实时推送并提供轮询兜底。
...
- HITL 当前支持**最小人工接管治理**：运行中的任务可通过 `POST /projects/{id}/overrides` 进入 `HUMAN_OVERRIDE`，override 带 owner/TTL lease；同任务同 scope 的未过期 lease 若 owner 不同会返回 `409` 并进入 `/projects/{id}/conflicts` 队列，`/status/panel` 会展示 open/resolved 冲突并可直接调用 resolve API 标记处理完成，执行器只在安全检查点应用未过期 override。
...
- 运维 readiness 当前支持 `GET /ready`：会检查配置有效性、auth token 配置、存储/数据目录、file-store service-state 完整性、Postgres store 连接、migrations checksum、domain projection tables 与 legacy 状态版本、artifact/sandbox 根目录可写性、Git workspace provider 的 repo/base ref 可用性以及 runtime provider 配置；`RuntimeProvider=container` 会校验容器 image 配置、本地 container binary 可执行性和资源/tmpfs 配置合法性，但不会 pull image、inspect image 或启动容器；配置 remote Git 后，readiness 可 clone 缺失 repo 并按需 fetch。
...
# 或随 release-check 一起执行
RUN_CONTAINER_SMOKE=1 bash scripts/release-check.sh
```

容器 runtime 相关环境变量：`MULTI_AGENT_RUNTIME_CONTAINER_BINARY`（默认 `docker`）、`MULTI_AGENT_RUNTIME_CONTAINER_IMAGE`、`MULTI_AGENT_RUNTIME_CONTAINER_NETWORK`（默认 `none`）、`MULTI_AGENT_RUNTIME_CONTAINER_USER`、`MULTI_AGENT_RUNTIME_CONTAINER_READONLY_ROOTFS`（默认 `true`）、`MULTI_AGENT_RUNTIME_CONTAINER_WORKDIR`（默认 `/workspace`）、`MULTI_AGENT_RUNTIME_CONTAINER_CPUS`、`MULTI_AGENT_RUNTIME_CONTAINER_MEMORY`、`MULTI_AGENT_RUNTIME_CONTAINER_PIDS_LIMIT`、`MULTI_AGENT_RUNTIME_CONTAINER_TMPFS`（默认 `/tmp:rw,nosuid,nodev,noexec,size=64m`，多个条目用 `;` 分隔）、`MULTI_AGENT_RUNTIME_CONTAINER_ENTRYPOINT`、`MULTI_AGENT_RUNTIME_CONTAINER_COMMAND`。镜像约定为从 stdin 读取 `runtime.http.v1` JSON payload，正常结果写 stdout，诊断/错误写 stderr 并以非 0 退出；entrypoint 和 command 直接作为参数传递，不经 shell 展开。
