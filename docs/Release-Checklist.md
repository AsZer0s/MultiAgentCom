...
- `POST /projects/{id}/locks` 可提交 `lockMode=go_symbol`、`language=go`、`symbolKind=func|method|type|var|const` 和 `symbolName`，并只替换对应 Go 声明；`LOCKED BY HUMAN` 必须位于被锁定 symbol 或其 doc comment 内，缺失 Go 目标文件可由 locked content 的 package/import 上下文创建，同时合并锁内容所需 imports、移除替换后不再使用的普通 imports
