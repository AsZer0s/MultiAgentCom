...
- `RUN_POSTGRES_SMOKE=1` 且提供 `MULTI_AGENT_POSTGRES_DSN` 或 `MULTI_AGENT_TEST_POSTGRES_DSN` 时，验证 Postgres-backed store 可跨服务重启恢复且 `/ready` 包含 `postgresStore` 检查；默认不执行、不阻塞普通 CI
...
- `MULTI_AGENT_STORE_PROVIDER=postgres` 且配置 `MULTI_AGENT_POSTGRES_DSN` 后，`GET /ready` 包含 `postgresStore` 检查；连接失败、schema ensure 失败、已有 JSONB state 损坏或 `version` 不支持时返回 `503`
